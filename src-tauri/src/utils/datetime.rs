    ...
    S: Serializer,
{
    match dt {
        ...
        Some(dt) => dt.format(FORMAT).to_string().serialize(serializer),
    }
}
...
mod tests {
    ...
    fn test_serialize_dt() {
        ...
        let now = Utc::now();
        let test_dt = TestSerializeDateUtc { date: Some(now) };
        let expected =
    ...
    fn test_serialize_dt_local() {
        let now = Local::now();
        let test_dt = TestSerializeDateLocal { date: Some(now) };
        let format = "%Y-%m-%d %H:%M:%S";
        let expected =
            "{\n  \"date\": \"".to_string() + now.format(format).to_string().as_str() + "\"\n}";
        let result = serde_json::to_string_pretty(&test_dt);
    ...
    fn test_deserialize_dt() {
        ...
        "#;
        let expected: DateTime<Utc> = Utc.with_ymd_and_hms(2024i32, 10, 19, 12, 23, 44).unwrap();
    ...
    fn test_deserialize_dt_local() {
        ...
        let expected: DateTime<Local> =
            Local.with_ymd_and_hms(2024i32, 10, 19, 12, 23, 44).unwrap();
